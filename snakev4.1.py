...
#Touch the side and die code!
    while x > -210 and x < 210 and y > -210 and y < 210:
        ...
        testwritingdata(str(x),str(y),str(foodcoord[0]),str(foodcoord[1]),str(game_continue))
...
def testwritingdata(x,y,foodx,foody,win):
    learning_file = open('learning_results.csv','a')
    learning_file.writelines(x + ',')
    learning_file.writelines(y +',')
    learning_file.writelines(foodx +',')
    learning_file.writelines(foody + ',')
